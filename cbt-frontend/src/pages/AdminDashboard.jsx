...
// ⭐ NEW: Helper function to convert data to CSV format
const convertToCsv = (data) => {
    if (!data || data.length === 0) {
        return '';
    }

    // Define the order and mapping for headers to ensure consistency and readability
    const headersMap = {
    ...
    const headerRow = readableHeaders.map(header => `"${header.replace(/"/g, '""')}"`).join(',');
    ...
    const csvRows = data.map(row => {
        return headers.map(header => {
            ...
            // Ensure section display is correct for CSV
            if (header === 'section') {
                value = renderSectionDisplay(value);
            }
            // For subject scores breakdown, handle newlines and commas
            if (header === 'formattedSubjectScores') {
                if (typeof value === 'string') {
                    // Escape double quotes and enclose in quotes if it contains commas, newlines, or quotes
                    return `"${value.replace(/"/g, '""')}"`;
                }
            }
            // For all other string values, escape double quotes and enclose if contains commas or quotes
...
function AdminDashboard() {
    ...
    receiptRef = useRef(); // For single payment receipt
    statementRef = useRef(); // For payment statement
    ...
    const handleDownloadResults = async (downloadType) => {
        ...
        try {
            ...
            if (downloadType === 'currentView') {
                dataToDownload = processedResults; // Already filtered and paginated for display
                filenamePrefix = 'Current_Page_Results';
            } else if (downloadType === 'allFiltered') {
                filenamePrefix = 'All_Filtered_Results';
                // Construct filters without pagination parameters for fetching all
                const allResultsFilterParams = new URLSearchParams();
                if (selectedResultsClassLevel) {
                    allResultsFilterParams.append('classLevel', selectedResultsClassLevel);
                }
                if (selectedResultsSubClassLevel) {
                    allResultsFilterParams.append('section', selectedResultsSubClassLevel);
                }
                if (isSeniorSecondaryClass(selectedResultsClassLevel) && selectedResultsDepartment) {
                    allResultsFilterParams.append('areaOfSpecialization', selectedResultsDepartment);
                }
                if (selectedDateTaken) {
                    allResultsFilterParams.append('dateTaken', selectedDateTaken);
                }
                if (studentIdFilter.trim()) {
                    allResultsFilterParams.append('studentId', studentIdFilter.trim());
                }

                const filterQueryString = allResultsFilterParams.toString();
                const resultsEndpoint = `/results${filterQueryString ? `?${filterQueryString}` : ''}`;

                setFeedbackMessage('Fetching all filtered results for download...');
                const response = await api.get(resultsEndpoint);
                const rawAllResults = response.data || [];
                dataToDownload = groupResultsByStudentAndExam(rawAllResults); // Process the raw data
                setFeedbackMessage('Results data fetched. Preparing download...');
            } else {
                setError('Invalid download type specified.');
                return;
            }

            if (dataToDownload.length === 0) {
                setFeedbackMessage('No results to download for the selected criteria.');
                return;
            }
            ...
            setFeedbackMessage('Results downloaded successfully!');
        } catch (err) {
            console.error("Download results error:", err);
            setError(err.response?.data?.message || err.message || 'Failed to download results.');
        }
    };
    ...
    return (
        <div className="container">
            ...
            <section className="section">
                ...
                <div className="resultsFilter">
                    ...
                    <div className="formGroup">
                        ...
                        <input
                            ...
                            onChange={(e) => setSelectedResultsDateTaken(e.target.value)}
                            className="input"
                        />
                    </div>
                ...
                {/* ⭐ NEW: Download Buttons for Results */}
                <div className="download-buttons-group" style={{ marginBottom: '1rem', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                    <button className="submitButton downloadButton" onClick={() => handleDownloadResults('currentView')}>
                        Download Current Page Results (CSV)
                    </button>
                    <button className="submitButton downloadButton" onClick={() => handleDownloadResults('allFiltered')}>
                        Download All Filtered Results (CSV)
                    </button>
                </div>

                {/* Existing "Print Results" button */}
...
.download-buttons-group /* ⭐ NEW: Hide download buttons for print */
{
...
/* Styles for new download buttons */
...
.download-buttons-group .downloadButton {
    background-color: #28a745; /* Green for download */
    border-color: #28a745;
}

.download-buttons-group .downloadButton:hover {
